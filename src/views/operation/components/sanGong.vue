    ...
    <el-col :xl="8" :lg="8" :md="8" :sm="10" :xs="24">
      ...
      <el-card v-loading="isLoading">
        <el-form label-width="100px">
          ...
          <el-form-item label="会员id">{{ formData.playId }}</el-form-item>
          ...
          <el-form-item label="当局底分">{{ formData.anteAmount | betNumberFormatter }}元</el-form-item>
          <el-form-item label="当局输赢">{{ formData.result | betNumberFormatter }}元</el-form-item>
          <el-form-item label="当局税收">{{ formData.dutyAmount | betNumberFormatter }}元</el-form-item>
        </el-form>
      </el-card>
    </el-col>
    ...
    <el-col :xl="16" :lg="16" :md="16" :sm="14" :xs="24">
      ...
      <el-card v-loading="isLoading">
        ...
        >
          ...
          <el-table-column label="抢庄倍数" prop="callType">
            <template slot-scope="{row}">
              {{row.isDealer?row.callType:'' }}
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    queryRecords() {
      this.isLoading = true;
      operationRequest
        .getSanGongDetails({ id: this.recordId })
        .then(res => {
