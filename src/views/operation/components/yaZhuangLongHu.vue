    ...
    <el-col :xl="8" :lg="8" :md="8" :sm="10" :xs="24">
      ...
      <el-card v-loading="isLoading">
        <el-form label-width="100px">
          ...
          <el-form-item label="是否上庄：">{{ formData.isDealer === true ? '是' : '否' }}</el-form-item>
          ...
          <el-form-item label="中奖总金额：">
              <el-tag>
                  {{ formData.goal | betNumberFormatter}}
              </el-tag>
          </el-form-item>
    ...
    <el-col :xl="16" :lg="16" :md="16" :sm="14" :xs="24">
      ...
      <el-card v-loading="isLoading">
        ...
        >
          ...
          <el-table-column label="赔率" prop="odd">
            <template slot-scope="{row}">
              {{row.odd | betNumberFormatter}}
            </template>
          </el-table-column>
          ...
          <el-table-column label="输赢">
            <template slot-scope="{row}">
              <el-tag type='success' v-if="row.isWin">赢</el-tag>
              <el-tag type='error' v-else>输</el-tag>
            </template>
          </el-table-column>
        </el-table>
      </el-card>
      <el-card v-loading="isLoading">
        ...
        <el-table
          ...
          :data="table2Data"
        >
          <el-table-column label="龙">
            <template slot-scope="{row}">
              {{row.pressLongColor | pokerColorFilter}}{{row.pressLongFace | pokerFaceFilter}}
            </template>
          </el-table-column>
          <el-table-column label="虎">
            <template slot-scope="{row}">
              {{row.pressHuColor | pokerColorFilter}}{{row.pressHuFace | pokerFaceFilter}}
            </template>
          </el-table-column>
        </el-table>
      </el-card>
...
export default {
  ...
  methods: {
    queryRecords() {
      this.isLoading = true;
      operationRequest.getYZLHDetails({ id: this.recordId }).then(res => {
        this.isLoading = false;
        if (res.code == 200) {
          this.formData = res.data;
          this.table1Data = res.data.detail;
          this.table2Data = [res.data.controlPokerVo];
        } else {
    ...
    displayHandicaps(row) {
      // 下注盘口 1：龙、2：押庄赢、3：押庄输、4：虎、5：龙黑、6：龙红、7：龙梅、8：龙方、9：和、10：虎黑、11：虎红、12：虎梅、13：虎方
      if (row.betTarget == 1) {
        return "龙";
      }
      if (row.betTarget == 2) {
