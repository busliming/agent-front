    ...
    <el-col :xl="8" :lg="8" :md="8" :sm="10" :xs="24">
      ...
      <el-card v-loading="isLoading">
        <el-form label-width="100px">
          ...
          <el-form-item label="当局桌号">{{ formData.tableNum}}</el-form-item>
          <el-form-item label="局号">{{ formData.tableNum }}</el-form-item>
          <el-form-item label="开始时间">{{ formData.startTime }}</el-form-item>
          <el-form-item label="结束时间">{{ formData.endTime }}</el-form-item>
          <el-form-item label="下注金额">{{ formData.betMoney | betNumberFormatter }}元</el-form-item>
          <el-form-item label="中奖总金额">{{ formData.awardAmount | betNumberFormatter }}元</el-form-item>
    ...
    <el-col :xl="16" :lg="16" :md="16" :sm="14" :xs="24">
      ...
      <el-card v-loading="isLoading">
        ...
        >
          ...
          <el-table-column label="抢庄倍数" prop="callType"></el-table-column>
          <el-table-column label="下注倍数">
            <template slot-scope="{row}">
              {{row.isDealer?'':row.betOdds }}
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    queryRecords() {
      this.isLoading = true;
      operationRequest
        .getQiangeZhuangDetails({ id: this.recordId })
        .then(res => {
          this.isLoading = false;
          if (res.code == 200) {
            this.formData = res.data;
            this.tableData = res.data.patterns;
          } else {
